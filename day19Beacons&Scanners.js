...
Foe part 1 find the number of beacons.  Scanners can see all beacons within 1000 units
...
const solver = (untriedSolvedDataArr, unsolvedIdxSet, scannerData, solution = new Set()) => {
    ...
    while (unsolvedIdxSet.size > 0 && moreSolved) { // keep solving until no more are solved with this solution
        ...
        for (const next of unsolvedArr) {
            ...
            if (newSolved !== null) {
                untriedSolvedDataArr.push(newSolved)  
                solution = union(solution, scanSet(newSolved))
                moreSolved = true
...
// other scanners, from 1 onwards, are unsolved
    ...
    const result = union(scanSet(scannerData[0]), solver([scannerData[0]], unsolvedSet, scannerData))
    return result.size // part1, the number of beacons
}
